...
// Returns the ticket of the environment with the given envid.
// if envid is 0 it returns the ticket of the current environment.
static envpriority_t
sys_get_env_priority(envid_t envid)
{
	...
	return e->env_priority;
}

static int
sys_lower_priority(int priority)
{
	if (priority < ENV_MAX_PRIORITY || priority > ENV_MIN_PRIORITY ||
